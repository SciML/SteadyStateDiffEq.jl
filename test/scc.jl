...
using SciMLBase: SciMLBase, SCCNonlinearProblem
...
@testset "SSRootfind SCC solve" begin
    @testset "iip=$iip vector=$use_vector alg=$alg" for iip in (false, true),
            use_vector in (false, true), alg in (SSRootfind(), SSRootfind(NewtonRaphson()))
    ...
    @testset "Ordinary steady-state wrapping" for alg in (SSRootfind(), SSRootfind(NewtonRaphson()))
